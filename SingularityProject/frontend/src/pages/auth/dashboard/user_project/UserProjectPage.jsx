...
export default function UserProjectPage() {
  ...
  const handleEdit = up => {
    navigate(`/dashboard/user_projects/edit/${up.id}`);
  };
  ...
  const handleDelete = async up => {
    ...
    await API.delete(`/user_projects/${up.id}/`);
    setUserProjects(cur => cur.filter(x => x.id !== up.id));
  };
      ...
      <div className="py-10 px-4 sm:px-6 lg:px-8 bg-gradient-to-b from-gray-50 to-white min-h-screen">
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center justify-between mb-8">
            <div>
              <h1 className="text-3xl font-extrabold text-gray-900">Manage Project Assignments</h1>
              <p className="text-sm text-gray-500 mt-1">Assign users to projects and manage their roles</p>
            </div>
            <button
              onClick={() => navigate('/dashboard/user_projects/new')}
                ...
                <tbody className="divide-y divide-gray-100">
                  {userProjects.map(up => (
                    <tr key={up.id} className="hover:bg-gray-50 transition">
                      ...
                      <td className="px-6 py-4">{up.project.title}</td>
