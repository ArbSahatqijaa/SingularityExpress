...
const NotificationBell = () => {
  ...
  // Handle the removal of a single notification
  const handleClearNotification = (e, notificationId) => {
    e.stopPropagation(); // Prevent notification click event
    clearNotification(notificationId);
  };
  
  // Group notifications by sender to improve visual organization
  const groupedNotifications = notifications.reduce((acc, notification) => {
    const senderKey = notification.senderId || 'unknown';
    if (!acc[senderKey]) {
      acc[senderKey] = [];
    }
    acc[senderKey].push(notification);
    return acc;
  }, {});
